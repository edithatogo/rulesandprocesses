...
| RuleSpec NZ [#80](https://github.com/TheAxiomFoundation/rulespec-nz/pull/80) | Closes [#79](https://github.com/TheAxiomFoundation/rulespec-nz/issues/79) | Both In Progress | `CHANGES_REQUESTED`; hosted `validate / validate` and `validate / validate (nz)` fail while shard, roadmap, and quality checks pass; a signing key is still required; local compile/Ruff/270-test evidence is not upstream CI | Maintainer resolves hosted failures, signs manifest, and reviews/merges/declines |
